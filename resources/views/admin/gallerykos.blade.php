...
@section('title', 'Gallery Kamar - {{ $kos->nama }} - {{ $kamar->nama }}')

@push('style')
<style>
.gallery-img {
    max-width: 150px; !important /* Ubah sesuai kebutuhan */
    max-height: 150px; !important /* Ubah sesuai kebutuhan */
    object-fit: cover;
}
.alert-dismissible {
    position: fixed;
...
<div class="dashboard-content">
    <h2 class="mb-4">Gallery Kamar: {{ $kamar->nama }} (Kos: {{ $kos->nama }})</h2>
    <div class="d-flex justify-content-between mb-3">
        <h5>List Gambar</h5>
        <a href="{{ route('kos.detail', $kos->id) }}" class="btn btn-secondary">Kembali ke Detail Kos</a>
    </div>
    ...
    <!-- Image Upload Form -->
    <div class="mb-4">
        <form id="uploadImageForm" action="{{ route('kos.gallery.store', [$kos->id, $kamar->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="mb-3">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function refreshTable() {
        fetch('{{ route('kos.gallery.data', [$kos->id, $kamar->id]) }}', {
            headers: {
        ...
        .then(data => {
            ...
            data.forEach(item => {
                ...
                row.innerHTML = `
                    <td>${item.id || '-'}</td>
                    <td><img src="${item.url}" alt="${item.nama_file}" class="gallery-img" style="max-width:150px;"></td>
    ...
    function attachButtonListeners() {
        document.querySelectorAll('.delete-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                document.getElementById('deleteImageForm').action = `{{ route('kos.gallery.destroy', [$kos->id, $kamar->id, 0]) }}`.replace('0', id);
            });
        });
    }
