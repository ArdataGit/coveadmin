    ...
    <div class="dashboard__table table-responsive">
        <table class="table">
            ...
            <tbody id="kamar-table-body">
                @forelse ($kosDetails as $item)
                    <tr data-id="{{ $item->id }}">
                        <td>{{ $item->id }}</td>
                        <td>{{ $item->nama }}</td>
                        <td>{{ $item->tipe_kos ? $item->tipe_kos->nama : '-' }}</td>
                        <td>{{ $item->lantai ? $item->lantai->nama : '-' }}</td>
                        <td>{{ $item->quantity }}</td>
                        <td>{{ $item->jenis_kos }}</td>
                        <td>
                            @if (is_array($item->fasilitas_ids) && !empty($item->fasilitas_ids))
                                @foreach ($item->fasilitas_ids as $fas_id)
                                    @php
                                        $fasilitas = $fasilitas->firstWhere('id', $fas_id);
                                    @endphp
                                    @if ($fasilitas)
                                        <span class="badge bg-primary me-1">{{ $fasilitas->nama }}</span>
                                    @endif
                                @endforeach
                            @else
                                <span>-</span>
                        ...
                        <td>
                            <button class="btn btn-sm btn-warning edit-btn" 
                                ...
                                data-dekat_dengan="{{ $item->dekat_dengan }}"
                                data-fasilitas_ids="{{ json_encode($item->fasilitas_ids) }}"
                            ...
                            <a href="{{ route('kos.gallery', [$kos->id, $item->id]) }}" class="btn btn-sm btn-info">
                ...
                <form id="addKamarForm" action="{{ route('kos.detail.store', $kos->id) }}" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="dekat_dengan" name="dekat_dengan"></textarea>
                            @error('dekat_dengan')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                ...
                <form id="editKamarForm" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="edit_dekat_dengan" name="dekat_dengan"></textarea>
                            @error('dekat_dengan')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function refreshTable() {
        ...
        .then(data => {
            ...
            data.forEach(item => {
                if (!item) return;
                const fasilitasIds = Array.isArray(item.fasilitas_ids) ? item.fasilitas_ids : JSON.parse(item.fasilitas_ids || '[]');
                const fasilitasChips = fasilitasIds.map(id => {
                    const nama = fasilitasMap[id] || 'Unknown';
                    return `<span class="badge text-white bg-primary me-1">${nama}</span>`;
                }).join('');
                ...
                row.innerHTML = `
                    ...
                    <td>${item.nama || '-'}</td>
                    <td>${item.quantity || '-'}</td>
                    <td>${item.lantai && item.lantai.nama ? item.lantai.nama : '-'}</td>
                    <td>${fasilitasChips || '-'}</td>
                    <td>${item.deskripsi || '-'}</td>
                    <td>${item.jenis_kos || '-'}</td>
                    <td>${item.tipe_kos && item.tipe_kos.nama ? item.tipe_kos.nama : '-'}</td>
    ...
    function attachButtonListeners() {
        document.querySelectorAll('.edit-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                document.getElementById('edit_dekat_dengan').value = dekat_dengan;
